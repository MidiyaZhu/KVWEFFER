...
from dataset2 import get_dataloadersmallset
from util import save_checkpoint,one_hot,iter_product,clip_gradient,load_model
from sklearn.metrics import accuracy_score,f1_score
import loss as loss
from model import primary_encoderwithksentimentatt2,weighting_network
import math

from transformers import AdamW,get_linear_schedule_with_warmup

os.environ["CUDA_VISIBLE_DEVICES"] = "1"
def train(device,epoch,train_loader,model_main,model_helper,loss_function,optimizer,lr_scheduler,log,modeltypelist,modelpath,bert_all):
    ...
    for idx,batch in enumerate(train_loader):
        ...
        emo_pred_1,supcon_feature_1 = model_main(text,attn,modeltypelist,modelpath,bert_all)
...
def test(device,epoch,test_loader,model_main,model_helper,loss_function,log,modeltypelist,modelpath,bert_all):
    model_main.eval()
    model_helper.eval()
    test_loss = 0
    total_epoch_acc_1,total_epoch_acc_2 = 0,0
...
def lcl_train(device,log,smallset,smallsetpkl,modeltypelist,modelpath,bert_all):
    ...
    # log.param.dataset = "isear"
    train_data,valid_data,test_data = get_dataloadersmallset(smallset,smallsetpkl,log.param.batch_size,log.param.dataset,w_aug=True,label_list=log.param.label_list)
    ...
    model_main = primary_encoderwithksentimentatt2(log.param.batch_size,log.param.hidden_size,log.param.emotion_size,log.param.model_type)
    for name, parameter in model_main.compat_model1.named_parameters():
    ...
    test_emo_f1_score_1, test_emo_accuracy_1, test_emo_f1_score_2, test_emo_accuracy_2 = float(0), float(0), float( 0), float(0)
    for epoch in range(1, log.param.nepoch + 1):

        train_loss_1,train_loss_2,train_acc_1,train_acc_2,train_acc_curve_1,train_acc_curve_2 = train(device,epoch,train_data,model_main,model_helper,losses,optimizer,lr_scheduler,log,modeltypelist,modelpath,bert_all)

        val_acc_1,val_acc_2,val_f1_1,val_f1_2,val_save_pred,val_acc_curve_1,val_acc_curve_2 = test(device,epoch,valid_data,model_main,model_helper,losses,log,modeltypelist,modelpath,bert_all)

        test_acc_1,test_acc_2,test_f1_1,test_f1_2,test_save_pred,test_acc_curve_1,test_acc_curve_2 = test(device,epoch,test_data,model_main,model_helper,losses,log,modeltypelist,modelpath,bert_all)
        ...
        if is_best:
            ...
            test_emo_f1_score_1 = test_f1_1
            test_emo_accuracy_1 = test_acc_1
            test_emo_f1_score_2 = test_f1_2
            test_emo_accuracy_2 = test_acc_2
            ## load the model
            ...
            fp.close()
    return  test_emo_f1_score_1, test_emo_accuracy_1,test_emo_f1_score_2,test_emo_accuracy_2
...
if __name__ == '__main__':
    ...
    bert_all = torch.load(
        r'../mappingmodel/Bert/WordEmbeddingList.pt')
    ...
    for param_com in param_list[1:]: # as first element is just name
        ...
        print('lr ', log.param.main_learning_rate)
        # log.param.dataset="isear"
        ...
        for set in range(len(smallset)):
            ...
            if smallset[set] == 100:
                smallsetpkl = [['cr_waug_preprocessed_bert.pkl','cr_waug_preprocessed_bert.pkl','cr_waug_preprocessed_bert.pkl','cr_waug_preprocessed_bert.pkl','cr_waug_preprocessed_bert.pkl'],

                         
                ]
          
            for n in range(len(smallsetpkl)):

                for m in range(len(modellist)):
                    testacc1,testacc2,testf11_marco,testf12_marco,testf12_weight,testf11_weight=[],[],[],[],[],[]
                    for mset in range(len(smallsetpkl[n])):
                        test_emo_f1_score_1, test_emo_accuracy_1,test_emo_f1_score_2,test_emo_accuracy_2=lcl_train(device,log,smallset[set],smallsetpkl[n][mset],modeltypelist[m],modellist[m],bert_all)
